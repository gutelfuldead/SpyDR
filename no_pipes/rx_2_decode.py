...
interleave_out = np.load(FILE_NAME_IN)
# mat_dim = int(math.sqrt(len(interleave_out)))

# # ################
# # # De-Interleaver
# # ################
# deinterleave_mat = np.zeros((mat_dim,mat_dim), dtype=np.int)
# output_stream = np.zeros(mat_dim**2, dtype=np.int)
# for i in range(0,mat_dim):
#     for j in range(0,mat_dim):
#         deinterleave_mat[i][j] = interleave_out[i*mat_dim + j]
# for i in range(0,mat_dim):
#     for j in range(0,mat_dim):
#         output_stream[i*mat_dim + j] = deinterleave_mat[j][i]
